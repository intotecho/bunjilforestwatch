...
def getTiles(image, red, green, blue):
    crs = image.getInfo()['bands'][0]['crs']
    p05 = []
    p95 = []
    p05 = getPercentile(image, 5, crs)
    ...
    mapparams = {    'bands':  'red, green, blue', 
                     'min': '85,84,90',
                     'max': '251,242,242',
                     #'min': json.dumps([85, 84, 90]),
                     #'max': json.dumps([251, 242, 242]),
                     #'max': [p95['red'], p95['green'], p95['blue']],
                     'gamma': 1.2,
                }   
    print mapparams
    ...
    overlay = MakeOverlay(image.getMapId(vis_params))
    
    tilepath = ee.data.getTileUrl(mapid, 2, 4, 6)
    return tilepath
...
class TestEEService(unittest.TestCase):
    ...
    def TestGetTiles(self):
        ...
        print tileurl
        self.assertEqual(tileurl.startswith("https://earthengine.googleapis.com//map"), True, 'TestGetTiles failed')
