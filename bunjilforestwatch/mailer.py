...
def new_gladcluster_email(task, hosturl):
    thesender = []
    returnstr = ""
    thesender = mail_sender()
    ...
    try:
        ...
        message.body = ("\n"
                        ...
                        "Or forward this email to {5!s} with \"UNSUBSCRIBE\" in the subject.\n\n"
                        "This is an automatically generated email from Bunjil Forest Watch \n\n"
                        "{4!s}\n").format(user.name, captured_str, task_url, task.aoi.string_id().encode('utf-8'),
                                          hosturl, thesender, completion_date_str)
        ...
        try:
            ...
            returnstr = "SMTP ERROR - Could not send mail Sent mail to user: {0!s}, with email: {1!s} from sender: {2!s} with subject: {3!s}".format(
                user.name, message.to, thesender, message.subject)
            logging.error(returnstr)
            logging.debug(message.html)
            returnstr += "<br> {0!s}".format(message.html)
            return returnstr
    ...
    returnstr = "Sent mail to user: {0!s}, with email: {1!s} from sender: {2!s} with subject: {3!s}".format(user.name,
                                                                                                            message.to,
                                                                                                            thesender,
                                                                                                            message.subject)
    logging.info(returnstr)
