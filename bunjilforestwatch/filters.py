...
def url(ob, name=''):
    if ob == 'feeds':
        ...
        return webapp2.uri_for(ob, username=name.key.parent().string_id(), journal_name=name.name.decode('utf-8'))
    elif ob == 'blog-entry':
...
def user_area_url(username, area_name):
	return webapp2.uri_for('view-area', username=username.encode('utf-8'), area_name=area_name.encode('utf-8'))
'''
